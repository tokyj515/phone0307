<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            NotificationHistory # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            NotificationHistory
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <String label="SentAt" v-model="value.sentAt" :editMode="editMode" :inputUI="''"/>
